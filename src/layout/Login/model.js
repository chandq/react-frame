...
import api from '@/services/login' //获取接口
...
export default {
  ...
  effects: {
    *goHome({ payload }, { put, call, select }) {
      //第一个参数为action,即action.payload,是dispatch触发action接收的参数对象
      //const {username,password} = payload;//将参数解构出来
      try {
        //try代码块里一般用call触发一个异步请求逻辑，错误抛出到catch代码块。select用于查找本modal或全局modal下的state。
        ...
        if (resdata.data === 1) {
          ...
          yield put(routerRedux.push(`${basename}/home`)) //put可以发起一个本model的reducer方法也可以本modal外的或者发起页面跳转。
        } else {
    ...
    *showMsg({ payload }, { put, call, select }) {
      //测试subscriptions订阅函数的异步逻辑
      try {
  ...
  reducers: {
    ...
    redshowMsg(state, action) {
      //测试subscriptions订阅函数的reducer
      return {
        ...state,
  ...
  subscriptions: {
    ...
    keyEvent({ dispatch }) {
      //keyEvent名字不是必须，叫什么都可以
      key('⌘+up, ctrl+up', () => {
        //'⌘+up, ctrl+up' 具体意思查看keymaster API
